## 2026-09-21 — Runs vs Cover 0 can go the distance  [playcall, grader, football]
**Symptom:** Runs vs Bear Zero never produced an explosive (0%), though Cover 0 has nobody deep.
**Root cause:** The run grader had no "no deep help" term, so one missed tackle vs Cover 0 was just a normal gain.
**Fix:** Ruled in by Himkage 2026-09-21 ("yes, runs against Cover 0 can break explosive"). Canon: textbook Ch 7 "THE COVER 0 RISK: One missed tackle or one lost 1-on-1 matchup = touchdown. There is nobody behind the CB." A run that gains 10+ yds (through the box: past the LBs and any rolled safety) against a coverage whose corpus deep count is 0 is a BREAKAWAY. The grader sets `outcome.breakaway` (new OPTIONAL field; gradePlay signature unchanged) and game.ts turns it into the true distance to the goal. Sim: ONLY the Bear Zero column moved. Breakaway rate vs Bear Zero: IZ 6.7%, Power 7.5%, Outside Zone 11.7%, Counter 4.8%, Read Option 20.7%; zero against every other defense. Real drives: every breakaway scored a TD with yards equal to 100 minus the spot (e.g. 38 from the 62).
**Files:** src/playcall/grade.ts, src/playcall/game.ts.
**Gotcha:** ON A RUN, model.C IS NOT "WHO IS IN COVERAGE". The engine removes every BLOCKED defender from C, and WR stalks take the corners and sometimes the free safety, so counting deep ents in C showed ZERO deep help against Cover 3 and Cover 1. My first version read C and gave Cover 3 house calls (Stretch 8.1). Caught by diffing the full sim matrix and asking why cells outside the Bear Zero column had moved. Read coverage structure from the corpus (coverage.deep), not from the post-block model.
**The one knob:** THROUGH_THE_BOX_YDS = 10. At 12 yds the Read Option rate is about 13%, at 15 about 6%. **Himkage ruled 2026-09-21: keep 10** ("I'm fine with how it is"), after seeing that table. Do not retune it without asking.
**Sim caveat:** sim-playcall averages count a breakaway as a flat 78 yds (the grader cannot see the spot), so the Bear Zero averages in the sim are a ceiling. The game uses the real distance.

## 2026-09-21 — The coverage call now changes the run game; Mesh is quick by ruling  [playcall, engine, grader, football]
**Symptom:** Runs averaged the same yards against every defense but prevent. Box count was 7 for everything.
**Root cause:** Two layers. (1) ENGINE: the coverage was post-snap movement only, so it never changed pre-snap alignment. 4-3 Over with Cover 3 and with Cover 1 Robber resolved identically, SS parked deep. (2) BOT: boxCount used a fixed window of x 330..670 around the center of the FIELD, not the formation, so the 46 Bear's 9-tech WILL at x678 missed by 8px (8-man front counted 7) and a nickel apex at x330 counted in.
**Fix:** (1) HimkageVision 04b3711: presnapShell() runs before the fits in all three resolvers; any safety the coverage does not need deep rolls down, using the same assignDeep the post-snap drops use. Cover 0 rolls 2, Cover 1 and 3 roll 1, two-high shells roll 0, a prevent front never rolls. Straight down with x unchanged so blocking is byte-identical (the suite's own invariant). New tests pin the textbook counts; suite 5,783,191 checks, 0 failures. (2) boxCount now counts the way a QB does: anyone on the ball in a numbered technique (0-9, 2i, 4i) is in by definition; off-ball defenders are in when inside run-fit depth and between the offense's end men plus one OL split (attached TE extends it). (3) Mesh drop "5" -> "quick" in the corpus (HimkageVision 115677c) per Himkage's ruling, not a grader exception.
**Verified against the textbook's stated numbers, both run formations, both directions:** Cover 3 8, Cover 1 Robber 8, Cover 6 7, nickel Cover 2 / Quarters / Palms 6, Bear Zero 9 (8-man front + FS down in Cover 0), prevent 5. Sim: IZ vs Cover 3 3.0 and vs Robber 2.9 (was 3.8), vs the two-high nickel shells ~4.7 (was 3.8), vs Bear Zero 1.5, vs prevent 6.6; option still 3.9 vs Bear Zero. Passes unchanged except Mesh, now 9.0 vs A-Gap Fire and 8.6 vs Bear Zero.
**Files:** src/playcall/engine.ts (boxCount), scripts/sim-playcall.ts, engine/play-engine.core.js + engine/play-data.js (re-synced from HimkageVision 7290a94 / 115677c).
**Gotcha:** THE ENGINE'S DEPTH SCALE IS 10.4 PX/YD (the field overlay's), not the horizontal one. I first wrote the roll depth as "~7.7 yds" using the wrong scale and corrected it (7290a94). Describe engine placement relative to other players, not in yards, unless you convert with the right axis.
**Found, NOT fixed, flagged to Himkage:** runs vs Cover 0 NEVER go for an explosive (0% vs Bear Zero). Cover 0 has "no safety over the top", so a run that clears the first level should be a house call; the grader has no "no deep help" term. The box change made this more visible (IZ 3.2 -> 1.5) but did not cause it.

## 2026-09-21 — Playcall football fixes: PALMS callable and graded, Prevent graded as prevent, quick game from the corpus  [playcall, grader, football]
**Symptom:** Himkage: "fix palms and any other football related problem." Audit of the grader against the corpus and canon found three real errors and one structural gap.
**Root cause + Fix, each grounded in a named source:**
1. **PALMS scored neutral against everything.** The engine gained PALMS as a coverage in 7a23793, but no concept's beats/beaten_by named it, and the grader reads only those tags. The knowledge existed the whole time: canon `cover-4-palms` is Tier 1 in coverages.json AND flores.json. Fixed UPSTREAM in HimkageVision 336545b, three tags taken verbatim from the canon: smash.beaten_by += palms ("smash concept"), four-verticals.beats += palms ("#2 vertical with #3 vertical, 'palms it away'"), mills.beats += palms (mills unit: "palms/poach"). Flood, slants and mesh deliberately left neutral because the canon does not name them against palms. Then added PALMS to the defense deck on nickel-over (quarters' front, since canon calls it "Quarters' cousin") and to the bot's early-down and passing-down picks.
2. **PREVENT was graded as Cover 3,** because that is the umbrella it drops into, so it inherited Cover 3's tags backwards: Four Verticals gained 10 a snap and Mesh was its worst matchup, directly contradicting its own blurb. Canon: textbook Ch 6 "Protect the deep ball. Give up underneath."; CFB27 two-minute loadout "give the underneath, keep everything in front." Now a front whose corpus family is 'prevent' is graded on the MEASURED depth of the concept's primary ('*') route: 15+ yds taken away, 10 and under given up, between neutral. Every primary route in the corpus is 20+ or 9 and under, so the thresholds do not tune the result.
3. **Quick game was a hand-written set {slants, mesh, flood},** the only authored football in the grader. The corpus says only slants is drop 'quick'; mesh and flood are drop '5', and flood has a deep corner. Now `concept.drop === 'quick'`.
**Result (sim, 4000 rolls):** Smash vs Palms 3.8, Four Verticals vs Palms 10.0. Vs Prevent: Verts 3.8, Smash 3.6, Flood 3.1, Mesh 9.6, Slants 10.1, runs 5.7-5.9. Flood vs A-Gap Fire 2.8 with 21% sacks. **Side effect flagged to Himkage, not decided:** Mesh vs the blitzes fell from ~9 to ~5.5 because the corpus calls mesh a 5-step drop. If mesh should be a quick-game answer, the fix is the corpus's drop value in HimkageVision, not a grader exception.
**NOT FIXED, structural, needs a decision:** runs are flat across every defense (box count 7 for all but prevent) because the COVERAGE NEVER CHANGES PRE-SNAP ALIGNMENT in the resolved model. 4-3 Over with Cover 3 and with Cover 1 Robber resolve to identical positions (SS at ~9 yds, outside the box), so the "Robber" never rolls down. And boxCount's window is a fixed x 330..670: the 46 Bear's walked-up WILL at x678 misses it by 8px, so the defining 8-man front counts 7, while the nickel's apex at x330 counts in. The honest fix is coverage-aware alignment in the ENGINE (a single-high rotation puts a safety in the box) plus a box defined off the offense's end men, not a widened window, which would just be tuning numbers until they look right.
**Files:** src/playcall/grade.ts, src/playcall/catalog.ts, scripts/sim-playcall.ts, engine/play-data.js (re-synced from HimkageVision 336545b).
**Gotcha:** "WE HAVE NO IDEA WHAT PALMS IS" WAS NEVER TRUE. The canon had it at Tier 1 twice. What was missing was the tag in the ONE table the grader reads. When a consumer reads a narrow slice of the knowledge (here, concept.beats keys), new knowledge added elsewhere (a new coverage) is invisible to it until someone connects the two. Check the consumer's actual input, not whether the knowledge exists.

## 2026-09-21 — Playcall diagrams: END ZONE over the title, a tofu arrow, and a first-down line in the end zone  [playcall, render]
**Symptom:** Two reported by League Ops Pro from its vendored copy: (1) at ballOn 62 and similar, "END ZONE" drew straight through the play title and its subtitle; (2) "PLAYSIDE" ended in an empty box. A third found while verifying the fix: with an uncapped toGo, the yellow first-down line drew INSIDE the end zone.
**Root cause:** (1) field.ts centered the label in the band, and inside about the opponent 40 the band covers exactly the strip where the engine draws the title block (y 48 / 74 / 96). (2) The engine drew the arrow as a U+2192 text glyph with no font-family; browsers fall back to a system font that has it, but our resvg path is Barlow only with loadSystemFonts:false, and Barlow has no arrow. (3) field.ts drew the line at ballOn+toGo with no cap. Our own game never hits it (firstDownDistance switches to goal-to-go at the 10), but LO Pro now drives field.ts from ITS game state, so the file cannot assume its caller caps toGo.
**Fix:** (1) END ZONE now sits at both sidelines, never centered, and is skipped when the visible band is too thin to hold it. (2) Fixed UPSTREAM in HimkageVision (06e3c9c): the arrow is a stroked path, and the engine suite passes (5,778,040 checks, 0 failures). Then re-synced engine/. Did NOT patch engine/ locally, because Part H would correctly flag it as drift and HimkageVision would keep the bug. (3) No yellow line on goal-to-go; the goal line is the marker.
**Files:** src/playcall/field.ts, engine/play-engine.core.js (re-synced from HimkageVision 06e3c9c).
**Gotcha:** A TEXT GLYPH IN AN SVG IS A FONT DEPENDENCY, and it fails only in the renderer you did not test. Any symbol outside Barlow's coverage (arrows, bullets beyond the middle dot, emoji) must be drawn as a path if it will ever reach resvg. Verified: sim:playcall is byte-identical before and after, so all three are drawing fixes only. Checked by rendering frames at ballOn 62 both directions, 75 on a long title (FOUR VERTICALS), and 92 goal-to-go, and looking at them.

## 2026-09-20 — deploy-exe.sh, and the deploy check that could not fail  [ops, deploy]
**Symptom:** Supersedes the entry below. Ruling came back on the central-tool question: a per-repo COMMITTED `deploy-exe.sh` is canonical, not `deploy <app>` from `~/.deploy`. Reason worth keeping: `$HOME` is the one place that does not survive a machine move, which is exactly how the central tool was left on BotPC on 2026-09-19 while every MANUAL here still pointed at it. A committed script cannot be lost, and it runs from exe-host or a cloud session where a PowerShell `$PROFILE` function never could.
**Fix:** New `deploy-exe.sh`, matching the house style (maddojo's minimal shape, LO Pro's two hard-won lessons). Recipe is verbatim from the retired `apps.json` entry, not reinvented. Doc references updated in MANUAL.md (4 places), docs/playcall-beta.md and ../OPERATOR-MANUAL.md. Also pinned `*.sh text eol=lf` in `.gitattributes`, because `* text=auto` would hand a Linux host a CRLF shebang.
**TWO BUGS IN MY OWN SCRIPT, both found by RUNNING it and reading the output rather than watching it exit:**
1. **The boot check could not fail.** It grepped the out log for "iMoveChainz Bot is online" with no time bound. pm2 keeps the OUT log across restarts, not just the err log, so the grep matched the PREVIOUS boot instantly and printed a clean-deploy banner stamped 21:47:16 for a restart that had happened at 23:54. It would have passed a bot that crashed on startup. Fixed by stamping the moment before the restart and only accepting a banner at or after it.
2. **Then the fixed check cried wolf,** reporting "did NOT come up clean" against a bot that had booted perfectly one second earlier. The stamp was `date -u` while pm2 prefixes log lines in HOST LOCAL time (EDT), so the bound sat 4 hours in the future and nothing could ever satisfy it.
**Files:** deploy-exe.sh (new), .gitattributes, MANUAL.md, docs/playcall-beta.md, ../OPERATOR-MANUAL.md.
**Gotcha:** A VERIFICATION THAT CANNOT FAIL IS NOT A VERIFICATION, and the only way to find out which kind you wrote is to run it and read what it says. Both bugs were invisible from the source: version one looked correct and printed a real banner with real counts, it was just the wrong boot's. Two habits that caught it: compare the printed timestamp against when you actually restarted, and check `pm2 jlist` restart_time/uptime rather than trusting log text. Related and the same shape as LO Pro's `&&` lesson and the board's out/ baseline rule: build output and log buffers both outlive the thing you think you are measuring.

## 2026-09-20 — `deploy imovechainz` does not exist on this machine; the MANUAL said it did  [ops, deploy, migration-gap]
**Symptom:** Went to ship the engine re-sync the documented way and `deploy` was not found, in bash or PowerShell. MANUAL.md said to use it in two places and OPERATOR-MANUAL.md in a third.
**Root cause:** The `deploy <app>` tool is a BotPC artefact (`\Botpc\c\Users\Himkage\.deploy\`: deploy.ps1, apps.json, README.md, gx.ps1, plus a PowerShell `$PROFILE` function) and never came across in the 2026-09-19 migration. Same class as the settings.local.json allow lists in board row R2101: authored config that is not git-tracked did not travel, while everything in a repo did. Confirmed by hq-dashboard-ba against the canonical board: zero rows mentioned it, so it was invisible, not deferred.
**Fix:** Deployed by hand with the exact recipe from `apps.json` (fetch, `git reset --hard origin/main`, `npm ci`, `npm run build`, `pm2 restart imovechainz`), verified live. MANUAL.md now carries the measured state and those four commands so the next person is not stuck. The tool port itself is HQ OS work and is on their board.
**Files:** MANUAL.md.
**Gotcha:** DO NOT WRITE A DEPLOY SCRIPT IN THIS REPO to route around it. `apps.json` is an ECOSYSTEM registry that also covers Homegrown, and per-repo reinvention is the exact drift the tool was built to stop (the memory's own cautionary note is Homegrown going 4 days stale from hand-copying). Correction from hq-dashboard-ba worth keeping: `apps.json` needs NO re-pathing, unlike the allow lists, because every path in it is REMOTE (`/opt/homegrown-bot`, `/home/himkage/imovechainz-bot`, via the ssh alias `exe`). So the port is a verbatim 4-file copy plus a `$PROFILE` function, and only the profile edit is Himkage's.
**Also fixed here:** step 4 of "Ship a change" now verifies the LIVE corpus when a change touches `engine/`. The boot banner prints card and playbook counts but says nothing about the engine, so a stale vendored corpus boots looking perfectly healthy. That is precisely how the play-data.js drift survived two months.

## 2026-09-20 — Engine corpus re-synced, and drift is now a CHECK instead of a discipline  [playcall, engine, resync]
**Symptom:** Follow-up to the drift entry below. Himkage said bring it up to date and live, so the vendored corpus was re-synced, verified, and shipped.
**Root cause:** See the entry below. `engine/play-data.js` was two upstream commits stale since 2026-07-05.
**Fix:** Copied both engine files from `../HimkageVision`, both now match source byte for byte (LF-normalised). Corpus reports 8 coverages including palms, `mesh-under` depth 5, `shallow` depth 4, everything else unchanged (49 fronts, 43 formations, 12 schemes, 26 concepts, 35 routes, 455 plays). `npm run build` clean, `npm run smoke` PASS, then deployed.
**NEW: Part H of the smoke test.** `scripts/smoke-test.ts` now compares both vendored files against `../HimkageVision/` on every run and FAILS on drift. It normalises line endings first (this repo is CRLF on Windows, LF on exe-host, so raw bytes differ for identical content). It SKIPS cleanly when the source repo is not checked out beside this one, which is the case on exe-host, so deploys are unaffected. Proven before it was trusted: run against the drifted tree it failed and named `play-data.js` only, correctly leaving `play-engine.core.js` alone.
**Files:** engine/play-engine.core.js, engine/play-data.js, scripts/smoke-test.ts, engine/README.md.
**CORRECTION to the entry below, which overstated the damage.** That entry said the game "resolves MESH at a crossing depth the author corrected as wrong", which implies outcomes were wrong. They were not. `npm run sim:playcall` is BYTE-IDENTICAL before and after the re-sync: not one average, explosive rate, turnover rate or sack rate moved. The grader reads box count, free rushers and leverage at the point of attack; it never reads route depth. What was actually wrong was the ART: MESH and Shallow Cross were DRAWN crossing at 2 and 3 yards instead of 5 and 4. That still matters in a game whose entire claim is that the picture is real football, but it is a drawing defect, not a balance defect. PALMS is now in the corpus but is NOT a callable hand in `src/playcall/catalog.ts`, so it does nothing until someone adds it to the eight-deep defense deck.
**Gotcha:** THE SIM IS THE INSTRUMENT THAT SETTLES THIS CLASS OF QUESTION. Capture `npm run sim:playcall` BEFORE touching the corpus and diff it after. A corpus change that moves the art and not the numbers is indistinguishable from one that moves both until you actually diff, and the confident-sounding version of the claim is the wrong one. Same discipline as not comparing against a gitignored `out/` directory: render your own baseline or you are not testing anything.

## 2026-09-20 — The vendored engine corpus drifted for two months and prod is running the stale one  [playcall, engine, DRIFT]
**Symptom:** None visible, which is the problem. Playcall resolves MESH, one of its ten offense hands, at a crossing depth the engine's own author corrected as wrong on 2026-07-05, and cannot call PALMS at all. Nobody noticed because nothing checks.
**Root cause:** `engine/` is a VENDORED SNAPSHOT of HimkageVision, copied once at 8b17ca8 (2026-06-20) and never since. Two upstream commits landed after it and were never carried across: a8c9c91 (2026-07-05) "real mesh + shallow depths", which moved `mesh-under` from depth 2 to 5 and `shallow` from 3 to 4, and 7a23793 (2026-07-13) "add PALMS as a full coverage", taking coverages 7 to 8. `play-engine.core.js` still matches source byte for byte; only `play-data.js` drifted. Confirmed against the RUNNING process, not just the checkout: requiring the live corpus on exe-host reports 7 coverages, no palms, `mesh-under` depth 2.
**Fix:** NOT re-synced. Re-syncing changes live football behaviour in a bot Himkage plays, so it is his call, not a session's to do quietly. What IS done: `engine/README.md` now carries a Drift section naming the two missed commits and how to check, plus a Runtime section recording that playcall reaches nothing at runtime, plus the corrected source path (it still said `C:\iMoveChainz\HimkageVision`, which is BotPC and dead; HimkageVision now lives at `G:\Dev\iMoveChainz\HimkageVision` as its own git repo with its own remote).
**Files:** engine/README.md, docs/playcall-beta.md.
**Gotcha:** DRIFT SHOULD BE A CHECK, NOT A DISCIPLINE. A hash comparison against `../../HimkageVision/` in `npm run smoke` would have caught this on 2026-07-05. A vendored snapshot with no drift test will always rot, and it rots silently because the stale data is still perfectly valid data. When you do compare, NORMALISE LINE ENDINGS FIRST: this repo checks out CRLF on Windows and LF on exe-host, so raw md5s differ for byte-identical content and you will chase a third copy that does not exist. LF-normalised, the local vendored copy and the deployed one match exactly.
**CORRECTION to the 2026-09-20 entry below, and to `docs/playcall-beta.md`:** that entry repeated the beta doc's claim that playcall was "never QA'd by tapping in live Discord". THAT IS WRONG. Himkage has played it and it works. His words, relayed 2026-09-20: "I've used that version of Play Call and it works fine. I don't know what happened to the documentation of me testing it, but it works. It just hasn't been given out to massive people." The doc was written before he played it and never updated, so it was three months behind reality, and on 2026-09-20 it caused TWO sessions independently to report the feature as unproven while deciding its future. The only part still true is that it has not been given to a wide group. Lesson, now written into the beta doc itself: if you test something here, say so in the file. The next reader has no other way to know and will assume the worst.

## 2026-09-20 — STANDING: a cross-repo shared-library proposal is live on src/playcall/  [playcall, cross-repo]
**Symptom:** Not a bug. Recorded so the next session in here does not refactor a seam another repo is proposing to build on. `src/playcall/` has been untouched since b8b5e48 (2026-06-21) and reads as dormant. It is not.
**Root cause:** League Ops Pro has its own `src/games/playcall/` (~1,237 lines, 7 files) with a TSB-style probabilistic `payoff_matrix.ts`, a 4x4 play vocabulary, per-guild Firestore persistence and a leaderboard. Himkage asked whether the two should be one. LO Pro's session (league-ops-26-pro-db) asked us directly on 2026-09-20.
**Fix:** Measured rather than remembered, and answered: keep BOTH surfaces, share the outcome engine as one pure library. The proposed shared unit is `engine.ts` + `grade.ts` (470 lines) sitting on the vendored 348 KB `engine/`. `gradePlay(off, def, model, rng = Math.random)` is already pure with an injectable RNG, which is the seam. LO Pro would layer roster on top and keep its own Discord surface and storage. Taken to Himkage by LO Pro; the board row is theirs, not ours, deliberately (one decision, one row).
**Files:** src/playcall/engine.ts, src/playcall/grade.ts, engine/ (no code changed by this entry).
**Gotcha:** THREE things to know before touching this module.
(1) `gradePlay`'s SIGNATURE is under discussion. Widening it to take OVR and dev trait is the net-new work LO Pro would need. Do not change that signature casually, and do not "simplify" the injectable RNG away: it is the only reason the grader is shareable and it is what `npm run sim:playcall` rolls 4,000x per matchup.
(2) The DEPENDENCY RUNS TOWARD LO PRO, not away. Ours deliberately ignores rosters because this bot has no league DB, so outcomes are pure scheme-vs-coverage geometry. Porting ours wholesale INTO LO Pro would lose their roster-aware outcomes, which is a regression in a league product. The easy answer ("theirs is older, port ours across") is the wrong one.
(3) The property worth protecting is that nothing in the outcome is invented: every EV, variance and event chance is derived from geometry the engine measured, or from a canon `beats`/`beaten_by` tag. OVR and dev trait would be the FIRST inputs that are not measurements of the snap. If they ever land, they belong as a bounded modifier on the roll, not as a term inside the geometry.
**Also measured, worth keeping:** PM2 `imovechainz` on exe-host is online, 0 restarts, 58 days up, deployed HEAD 4014d20, playcall tree at b8b5e48. That is DEPLOYED AND REACHABLE, not proven in play: state is in-memory so there is zero usage record, and `docs/playcall-beta.md` still records it as never QA'd by tapping in live Discord. Do not describe it as battle-tested. Tenant shape is clean: zero guildIds, channel ids or 17-20 digit literals in `src/playcall/` or anywhere in `src/`. Single-guild by deployment, not by code.
